...
import { Music, Zap, ShieldCheck, ShoppingCart, Clock } from 'lucide-react';
...
const Spotify: React.FC = () => {
  ...
  const getPrice = (duration: string) => {
    ...
    return parseInt(publicSettings[`price_spotify_${duration}`]) || 0;
  };
  ...
  return (
    <div className="container" style={{ padding: '60px 20px 80px' }}>
      <div style={{ textAlign: 'center', marginBottom: '60px' }}>
         <h1 style={{ fontSize: '3rem', fontWeight: 900, marginBottom: '16px' }}>Spotify <span style={{ color: '#1db954' }}>Premium</span></h1>
         <p style={{ color: 'var(--text-muted)', fontSize: '1.2rem' }}>Thế giới âm nhạc trong tầm tay bạn. Chất lượng âm thanh tuyệt hảo.</p>
      </div>
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className="glass-panel" style={{ padding: '30px', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: '20px', transition: 'all 0.3s', border: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ background: 'rgba(29,185,84,0.1)', width: '50px', height: '50px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1db954' }}>
                <Music size={24} />
              </div>
              <div>
              ...
              <div style={{ marginTop: 'auto' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', color: '#1db954' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button 
                    className="btn" 
                    style={{ flexGrow: 1, fontWeight: 800, background: '#1db954', color: 'black' }} 
